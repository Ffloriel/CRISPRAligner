...
def process_sequence(
    ...
    for m in range(len(sequence) - 29):
        ...
        if (sequence[m:m+6] == check1 or 
            sequence[m+20:m+28] == check2):
...
def find_crispr_spacers(
    ...
    Args:
        ...
        prefix: Prefix for output file names
        results_dir: Directory to store results (defaults to ./Results)
    ...
    with open(results_dir / "AllCRR" / f"{prefix}AllCRR.fasta", "w") as all_file, \
         open(results_dir / "CRR1" / f"{prefix}CRR1.fasta", "w") as crr1_file, \
         open(results_dir / "CRR2" / f"{prefix}CRR2.fasta", "w") as crr2_file, \
         open(results_dir / "CRR4" / f"{prefix}CRR4.fasta", "w") as crr4_file, \
         open(results_dir / f"{prefix}Results.csv", "w") as csv_file, \
         open(results_dir / f"{prefix}Error.fasta", "w") as error_file:
        ...
        # Write headers
        csv_file.write("Strain ID, CRR1 Spacers, CRR2 Spacers, CRR4 Spacers, Total Spacers\n")
        error_file.write("Potential assembly errors found in the following spacers:\n\n")
